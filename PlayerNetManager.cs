...
internal class PlayerNetManager : ManagedSingleton<PlayerNetManager>
{
	...
	public string RawExtData => rawExData;
	...
	public void LoginRetrieveCollector1Req(NetDeviceInfo deviceInfo, Callback p_cb = null)
	{
		...
		{
			HasDeviceInfo = ((deviceInfo != null) ? ((sbyte)1) : ((sbyte)0)),
	...
	private string GetRoomID()
	{
		...
		string text = CapUtility.GetSubString("PVP[", "]", strRoomID, includeBeginTag: false).Trim();
		if (text.Length < 1)
		{
			text = CapUtility.GetSubString("PVE[", "]", strRoomID, includeBeginTag: false).Trim();
		}
	...
	public void StageEndReq(StageEndReq tStageEndReq, Action<StageEndRes> p_cb = null)
	{
		...
		if (ManagedSingleton<OrangeDataManager>.Instance.STAGE_TABLE_DICT.TryGetValue(tStageEndReq.StageID, out var value))
		{
			switch (value.n_TYPE)
			{
			case 9:
	...
	{
		_ = tIRequest is SetTutorialFlagReq;
	}
