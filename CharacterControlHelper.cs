...
public class CharacterControlHelper : ManagedSingleton<CharacterControlHelper>
{
	...
	public Vector3 ClimbingOffset => new Vector3(0.5f, 0f, 0f);
	...
	{
		WeaponStruct currentSkillObj = _refEntity.GetCurrentSkillObj();
		characterControlBase.CallPet(petId, isHurt: false, -1, null);
	...
	{
		if (ManagedSingleton<OrangeDataManager>.Instance.PET_TABLE_DICT.TryGetValue(petID, out var petTable))
		{
	...
	public T CallPet<T>(IPetSummoner summoner, OrangeCharacter _refEntity, int petID, int nSetNumID, bool linkPlayerBuff, bool followPlayer, bool activeNow = true, string acb = null, string cue = null, Vector3? pos = null) where T : PetControllerBase
	{
		...
		if (_refEntity.IsLocalPlayer || nSetNumID != -1)
		{
			...
			if (nSetNumID == -1)
			{
				if (!pos.HasValue)
				{
					StageUpdate.SyncStageObj(4, 4, _refEntity.sNetSerialID + "," + petID + "," + petCount, bNoCheckHost: true);
				}
				else
				{
					Vector3 value = pos.Value;
					StageUpdate.SyncStageObj(4, 4, _refEntity.sNetSerialID + "," + petID + "," + petCount + "," + value.x + "," + value.y + "," + value.z, bNoCheckHost: true);
				}
			}
