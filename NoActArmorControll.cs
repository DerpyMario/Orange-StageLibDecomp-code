...
public class NoActArmorControll : RideBaseObj, IManagedUpdateBehavior, IManagedLateUpdateBehavior, IAimTarget, ILogicUpdate
{
	...
	private void Start()
	{
		...
		{
			Transform[] target = base.transform.GetComponentsInChildren<Transform>(includeInactive: true);
	...
	private void CheckLockRange()
	{
		...
		while (bCheck1 || bCheck2)
		{
			...
			{
				if (mLockRangeObj.CheckLogicPos(Controller, bCheckNoOutRange: false, bCheckBottom: true))
				{
			...
			{
				if (tMasterPilotLockRangeObj.CheckLogicPos(MasterPilot.Controller, bCheckNoOutRange: true))
				{
	...
	public void LogicUpdate()
	{
		...
		if (MasterPilot != null)
		{
			if (MasterPilot.IsDead())
			{
				...
				{
					PlayRUSH04_LP(flag: false);
				}
				return;
			}
			if (!RUSH04_LP_play && efx_dash_root.isPlaying)
			{
				PlayRUSH04_LP(flag: true);
			}
		}
	...
	public override void SetStun(bool enable, bool bCheckOtherObj = true)
	{
		...
		if (MasterPilot != null)
		{
			MasterPilot.SetStun(enable, bCheckOtherObj: false);
		}
	}
