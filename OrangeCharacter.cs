...
public abstract class OrangeCharacter : StageObjBase, IManagedUpdateBehavior, IManagedLateUpdateBehavior, IAimTarget, ILogicUpdate
{
	...
	protected Setting _setting = new Setting(ModelSetup: true);
	...
	public bool UsingVehicle => _usingVehicle;
	...
	public HumanBase.AnimateId AnimateID => _animateID;

	public HumanBase.AnimateId AnimateIDPrev => _animateIDPrev;
	...
	public PlayerAutoAimSystem PlayerAutoAimSystem => _playerAutoAimSystem;
	...
	public bool IsStun => _stunStack > 0;
	...
	public ChargeShootObj ChargeObject => _chargeShootObj;
	...
	protected virtual void Start()
	{
		...
		if (tRefPassiveskill.HaveFlyMode() && tRefPassiveskill.GetFlyParam(out var skillTable))
		{
	...
	private void PlayerContinueAppear(bool bNeedChangeNUAFalse)
	{
		...
		CharacterMaterials.Appear(delegate
		{
			...
			if (GetCurrentWeaponObj().ChipEfx != null && GetCurrentWeaponObj().chip_switch)
			{
				...
				GetCurrentWeaponObj().ChipEfx.ActiveChipSkill(value: true);
			}
			InitImportantVar();
		});
	}
	...
	private IEnumerator ContinueEffectCoroutine(bool bNeedChangeNUAFalse, Action<bool> EndCB, bool bCallSCB = false)
	{
		...
		EndCB?.Invoke(bNeedChangeNUAFalse);
	}
	...
	protected virtual void Initialize()
	{
		...
		Transform[] target = _transform.GetComponentsInChildren<Transform>(includeInactive: true);
	...
	public void TeleportIn()
	{
		...
		StartCoroutine(ContinueEffectCoroutine(bNeedChangeNUAFalse: false, OCAppear, bCallSCB: true));
	}
	...
	public void RemovePlayerObjInfoBar()
	{
		if (objInfoBar != null)
		{
			...
			objInfoBar.gameObject.SetActive(value: false);
	...
	public virtual void UpdateValue()
	{
		...
		if (AnimatorModelShiftYOverride != null && AnimatorModelShiftYOverride.TryGetValue(CurMainStatus, out var value))
		{
	...
	protected void CreateBulletDetail(BulletDetails tBulletDetails)
	{
		...
		StageUpdate.SyncStageObj(4, 2, JsonConvert.SerializeObject(objFromPool, Formatting.None, new ObscuredValueConverter()), bNoCheckHost: true);
		...
		case MainStatus.HURT:
		{
			...
			CharacterMaterials.Disappear(delegate
			{
				...
				if (!bIsDeadEnd)
				{
					...
					Singleton<GenericEventManager>.Instance.NotifyEvent(EventManager.ID.STAGE_PLAYER_DESTROY_ED, this, p_param2: false);
				}
			});
			break;
		}
	...
	public int HurtPercent(float dmgPercent, WeaponType wpnType = WeaponType.Buster, string owner = "")
	{
		...
		StageUpdate.SyncStageObj(0, 0, JsonConvert.SerializeObject(BulletBase.tNetDmgStack, Formatting.None, new ObscuredValueConverter()), bNoCheckHost: true);
		return Hp;
	}
	...
	public void CreateBulletByLastWSTranform(SKILL_TABLE tSKILL_TABLE)
	{
		...
		if (tSKILL_TABLE.n_TRIGGER == 18)
		{
			...
			if (transform != null)
			{
				PushBulletDetail(tSKILL_TABLE, weaponStatus, transform, 0, lastCreateBulletShotDir, useExtraCollider: true, nSetDir);
			}
			else
			{
				PushBulletDetail(tSKILL_TABLE, weaponStatus, shootPosition, 0, lastCreateBulletShotDir, useExtraCollider: true, nSetDir);
			}
		}
		else if (tSKILL_TABLE.n_TRIGGER == 4)
		{
			...
			PushBulletDetail(tSKILL_TABLE, weaponStatus2, transform, 0, shotDir, useExtraCollider: true, nSetDir);
		}
		else if (transform != null)
		{
			PushBulletDetail(tSKILL_TABLE, lastCreateBulletWeaponStatus, transform, 0, lastCreateBulletShotDir, useExtraCollider: true, nSetDir);
		}
		else
		{
			PushBulletDetail(tSKILL_TABLE, lastCreateBulletWeaponStatus, shootPosition, 0, lastCreateBulletShotDir, useExtraCollider: true, nSetDir);
		}
	}
	...
	public void CloseChipEfx()
	{
		if (GetCurrentWeaponObj().ChipEfx != null && GetCurrentWeaponObj().chip_switch)
		{
			...
			GetCurrentWeaponObj().ChipEfx.ActiveChipSkill(value: false);
		}
	}
	...
	public IEnumerator SetKnockOutWithAttacker(OrangeCharacter attacker)
	{
		...
		if (!_isJack)
		{
			base.transform.SetParent(attacker.transform, worldPositionStays: true);
			_isJack = true;
			IgnoreGravity = true;
		}
	...
	public virtual void PlayerDead(bool bJustRemove = false)
	{
		...
		else
		{
			...
			NullHurtAction();
			RemovePlayerObjInfoBar();
			Singleton<GenericEventManager>.Instance.NotifyEvent(EventManager.ID.STAGE_PLAYER_DESTROY_ED, this, p_param2: true);
			CharacterMaterials.Disappear(delegate
			{
				bNeedUpdateAlways = false;
				SetActiveFalse();
	...
	public void PlayerPressJump()
	{
		...
		if (Controller.JumpUPThrough)
		{
			...
			{
				OrangeBattleUtility.ChangeLayersRecursively(ModelTransform, ManagedSingleton<OrangeLayerManager>.Instance.RenderSPEnemy, chageSelf: false);
	...
	public bool PlWallStopCheck(int dir)
	{
		if (!Controller.BelowInBypassRange && (bool)Solid_meeting(dir, 0f, Controller.wallkickMask))
		{
			...
			PlayerStopDashing();
			PlayerStopHovering(setFallStatus: false);
			SetHorizontalSpeed(0);
			return true;
		}
		return false;
	...
	public void PlayerHeldShoot()
	{
		...
		case WeaponType.Launcher:
			...
			PlayerShootBuster(GetCurrentWeaponObj(), isSkill: false, WeaponCurrent, 0);
	...
	public void PlayerPressChip()
	{
		...
		if (GetCurrentWeaponObj().ChipEfx != null)
		{
			if (!base.IsHidden)
			{
				GetCurrentWeaponObj().ChipEfx.ActiveChipSkill(tRefPassiveskill.bUsePassiveskill, ignoreChipActive: false);
			}
	...
	private void InitializeFSRaySplasher()
	{
		for (int i = 0; i < PlayerFSkills.Length; i++)
		{
			...
			_ = PlayerFSkills[i].BulletData.f_EFFECT_Z;
			...
			if (!MonoBehaviourSingleton<PoolManager>.Instance.ExistsInPool<SCH006Controller>(petTable.s_MODEL))
			{
				...
				_ = PlayerFSkills[i].weaponStatus;
	...
	private void ActivateFSRaySplasher()
	{
		...
		if ((bool)poolObj)
		{
			...
			poolObj.Set_follow_Player(this, linkBuffManager: false);
			poolObj.SetFollowEnabled(value: false);
			poolObj.ForceSetLocalPlayer(isLocalPlayer: true);
			poolObj.SetParams(pET_TABLE.s_MODEL, lifeTime, pET_TABLE.n_SKILL_0, weaponStatus, 0L);
			poolObj.SetActive(isActive: true);
		}
	}
	...
	public bool CheckUseSkillKeyTriggerEX2(int id)
	{
		if (id == 0 || id == 1)
		{
			CheckUseKeyTrigger((ButtonId)(8 + id), checkSkillStatus: false);
	...
	protected internal void PlayerShootBuster(WeaponStruct weaponStruct, bool isSkill, int id, sbyte lv = 0, Vector3? ShotDir = null, bool changeWeaponMesh = true, bool isWithLink = true)
	{
		...
		if (lv == 0 && !isSkill)
		{
			_ = weaponStruct.WeaponData.n_TYPE;
			if (useWeaponSE)
			{
				useWeaponSE = false;
	...
	public virtual void PlayerPressSelect()
	{
		...
		if (tRefPassiveskill.bUsePassiveskill)
		{
			...
			{
				PlayerWeapons[weaponCurrent].ChipEfx.ActiveChipSkill(tRefPassiveskill.bUsePassiveskill, ignoreChipActive: false);
			}
		}
		if (IsLocalPlayer)
		{
	...
	public void AnimationEnd()
	{
		...
		case MainStatus.TELEPORT_IN:
			if (CurSubStatus == SubStatus.TELEPORT_POSE)
			{
				...
				{
					BattleInfoUI.Instance.SwitchOptionBtn(bShow: true, 2);
				}
			}
			break;
		case MainStatus.IDLE:
	...
	protected internal void ActivateMeleeAttack(WeaponStruct targetWeapon, SlashType slashType = SlashType.None)
	{
		...
		if (flag)
		{
			if ((bool)targetWeapon.SlashObject && (bool)targetWeapon.SlashEfxCmp)
			{
				...
				{
					targetWeapon.SlashEfxCmp.ActivateMeleeEffect(flag, slashType, ReversedQuaternion, isLeft: true);
				}
				else
				{
					targetWeapon.SlashEfxCmp.ActivateMeleeEffect(flag, slashType, NormalQuaternion, isLeft: false);
				}
			}
			...
			_listSlashCache[0].MeleeBullet.PlayUseSE(force: true);
		}
		else
		{
	...
	public override void SetStun(bool enable, bool bCheckOtherObj = true)
	{
		if (bCheckOtherObj)
		{
			...
			{
				SetStun(enable, bCheckOtherObj: false);
			}
			return;
		}
		if (enable)
	...
	public override void SetNoMove(bool enable, bool bCheckOtherObj = true)
	{
		if (bCheckOtherObj)
		{
			...
			{
				SetNoMove(enable, bCheckOtherObj: false);
			}
			return;
		}
		if (enable)
	...
	public override void SetBanAutoAim(bool enable)
	{
		...
		{
			BanAutoAim = true;
			PlayerAutoAimSystem.SetEnable(enable: false);
		}
	...
	public virtual bool EnterRideArmor(RideBaseObj targetRideArmor)
	{
		...
		if (_jumpThroughCoroutine != null)
		{
			OrangeBattleUtility.ChangeLayersRecursively(ModelTransform, ManagedSingleton<OrangeLayerManager>.Instance.RenderPlayer, chageSelf: false);
			_chargeShootObj.ChangeLayer(ManagedSingleton<OrangeLayerManager>.Instance.RenderPlayer, includeSelf: true);
		}
		Singleton<GenericEventManager>.Instance.NotifyEvent(EventManager.ID.ENTER_OR_LEAVE_RIDE_ARMOR, p_param: true);
		return true;
	}
	...
	public virtual void LeaveRideArmor(RideBaseObj targetRideArmor)
	{
		...
		UpdateSkillIcon(PlayerSkills);
		UpdateAimRangeByWeaponEvt(GetCurrentWeaponObj());
		Singleton<GenericEventManager>.Instance.NotifyEvent(EventManager.ID.ENTER_OR_LEAVE_RIDE_ARMOR, p_param: false);
	}
	...
	{
		_chargeShootObj.ChangeLayer(layer, includeSelf: true);
	}
