...
public class StageRewardUI : OrangeUIBase
{
	...
	private void Start()
	{
		...
		if (refStageEndRes != null)
		{
			...
			{
				expbarsub[i].gameObject.SetActive(value: true);
			}
			for (; i < expbarsub.Length; i++)
			{
				expbarsub[i].gameObject.SetActive(value: false);
			}
		...
		else
		{
			...
			{
				expbarsub[j].gameObject.SetActive(value: true);
			}
			for (; j < expbarsub.Length; j++)
			{
				expbarsub[j].gameObject.SetActive(value: false);
			}
		...
		if (!StageUpdate.gbIsNetGame)
		{
			if (ManagedSingleton<StageHelper>.Instance.nLastStageID != 0)
			{
				...
				case StageType.Activity:
					nShowType = 2;
					ActivityRoot.SetActive(value: true);
					TimeRoot.SetActive(value: false);
				...
				case StageType.BossRush:
					nShowType = 2;
					ActivityRoot.SetActive(value: false);
					TimeRoot.SetActive(value: true);
					CleanText.text = MonoBehaviourSingleton<LocalizationManager>.Instance.GetStr("STAGE_CLEAR_TIME");
					break;
		...
		for (int m = 0; m < CleanRank.Length; m++)
		{
			...
			EffectRoot[m].SetActive(value: false);
		}
		if (NewRecordImg != null)
		{
			NewRecordImg.gameObject.SetActive(value: false);
			RotateSelf[] componentsInChildren = NewRecordImg.transform.GetComponentsInChildren<RotateSelf>(includeInactive: true);
		...
		if (ScoreOutTxt != null)
		{
			ScoreOutTxt.gameObject.SetActive(value: false);
		}
		if (ManagedSingleton<StageHelper>.Instance.nLastStageID != 0)
		{
			...
			if (GoTextAll.Length > nShowType)
			{
				...
				for (int num7 = 0; num7 < 3; num7++)
				{
					...
					GoOkImgAll[nShowType][num7].gameObject.SetActive(value: false);
					GoEndImgAll[nShowType][num7].gameObject.SetActive(value: false);
				}
			}
		...
		else
		{
			for (int num8 = 0; num8 < 3; num8++)
			{
				...
				GoOkImgAll[nShowType][num8].gameObject.SetActive(value: false);
				GoEndImgAll[nShowType][num8].gameObject.SetActive(value: false);
			}
		...
		if (StageUpdate.gbGeneratePvePlayer)
		{
			...
			{
				PlayerObj[num9].gameObject.SetActive(value: false);
			}
			long num10 = 0L;
			int num11 = 0;
			...
			for (int num13 = 0; num13 < MonoBehaviourSingleton<OrangeBattleServerManager>.Instance.ListMemberInfo.Count && num13 < 2; num13++)
			{
				...
				PlayerObj[num11].gameObject.SetActive(value: true);
				...
				if (num14 > 50f)
				{
					ImageMVP[num11].SetActive(value: true);
					ImageNotMVP[num11].SetActive(value: false);
				}
				else
				{
					ImageMVP[num11].SetActive(value: false);
					ImageNotMVP[num11].SetActive(value: true);
				}
				...
				component2.SetOtherInfo(MonoBehaviourSingleton<OrangeBattleServerManager>.Instance.ListMemberInfo[num13].netSealBattleSettingInfo.MainWeaponInfo, CommonIconBase.WeaponEquipType.UnEquip, bHasWeapon: true, num15, bShowBackup: false);
				...
				if (MonoBehaviourSingleton<OrangeBattleServerManager>.Instance.ListMemberInfo[num13].netSealBattleSettingInfo.SubWeaponInfo.WeaponID != 0)
				{
					...
					component2.Setup(0, AssetBundleScriptableObject.Instance.m_iconWeapon, wEAPON_TABLE.s_ICON);
					component2.SetOtherInfo(MonoBehaviourSingleton<OrangeBattleServerManager>.Instance.ListMemberInfo[num13].netSealBattleSettingInfo.SubWeaponInfo, CommonIconBase.WeaponEquipType.UnEquip, bHasWeapon: true, num15, bShowBackup: false);
				}
				else
				{
					component2.Setup(0, "", "");
	...
	public void GoToHome()
	{
		if (listLTDescrCtrl.Count > 0)
		{
			for (int num = listLTDescrCtrl.Count - 1; num >= 0; num--)
			{
				...
				LeanTween.cancel(ref uniqueId, callOnComplete: true);
			}
	...
	private void SetStageRewardIcon(StageRewardCell.PerRewardCell tPerRewardCell, int nID)
	{
		...
		{
			tPerRewardCell.refRoot.SetActive(value: true);
		...
		{
			tPerRewardCell.refRoot.SetActive(value: false);
		}
	}
	...
	private void SetStageRewardItemButton(RewardButtonRef tItemButton, int nIndex)
	{
		...
		{
			tItemButton.SelfRoot.gameObject.SetActive(value: false);
			OrangeRareText.Rare rare = OrangeRareText.Rare.DUMMY;
			_ = MonoBehaviourSingleton<OrangeGameManager>.Instance.ServerUnixTimeNowUTC;
			...
			{
				tItemButton.RareRoot.SetActive(value: true);
				tItemButton.CompaignMark.SetActive(value: false);
			}
			else if (list.Contains(refStageEndRes.RewardEntities.RewardList[nIndex].RewardID))
			{
				tItemButton.RareRoot.SetActive(value: false);
				tItemButton.CompaignMark.SetActive(value: true);
			}
			else
			{
				tItemButton.RareRoot.SetActive(value: false);
	...
	{
		if (obj is LTDescrCtrl item)
		{
			listLTDescrCtrl.Remove(item);
		}
	}
	...
	public void StartExpUP()
	{
		...
		if (bEndRightNow)
		{
			MonoBehaviourSingleton<OrangeGameManager>.Instance.DisplayLvPerform(delegate
			{
				...
				{
					expbarsub[k].gameObject.SetActive(value: true);
				}
				for (; k < expbarsub.Length; k++)
				{
					expbarsub[k].gameObject.SetActive(value: false);
				}
	...
	private void StartShowItemButton(int nIndex)
	{
		if (bEndRightNow)
		{
			ItemButtons[nShowType][nIndex].SelfRoot.SetActive(value: true);
	...
	private void StartStageGo(int nIndex)
	{
		if (bEndRightNow)
		{
			GoOkImgAll[nShowType][nIndex].gameObject.SetActive(value: true);
			GoEndImgAll[nShowType][nIndex].gameObject.SetActive(value: true);
			...
			GoEndImgAll[nShowType][nIndex].gameObject.SetActive(value: false);
	...
	public void StartStageGos(int nIndex)
	{
		...
		{
			STAGE_TABLE sTAGE_TABLE = ManagedSingleton<OrangeDataManager>.Instance.STAGE_TABLE_DICT[ManagedSingleton<StageHelper>.Instance.nLastStageID];
			_ = new int[3] { sTAGE_TABLE.n_CLEAR1, sTAGE_TABLE.n_CLEAR2, sTAGE_TABLE.n_CLEAR3 };
			_ = new int[3] { sTAGE_TABLE.n_CLEAR_VALUE1, sTAGE_TABLE.n_CLEAR_VALUE2, sTAGE_TABLE.n_CLEAR_VALUE3 };
	...
	public void StartShowPlayer(int nIndex)
	{
		if (bEndRightNow)
		{
			PlayerObj[nIndex].gameObject.SetActive(value: true);
	...
	public void StartShowScoreOut()
	{
		if (bEndRightNow)
		{
			ScoreOutTxt.gameObject.SetActive(value: true);
	...
	public void StartShowNewRecord()
	{
		if (bEndRightNow)
		{
			NewRecordImg.gameObject.SetActive(value: true);
	...
	public void StartShowCleanRank()
	{
		...
		if (bEndRightNow)
		{
			...
			EffectRoot[nIndex].SetActive(value: true);
			CleanRankScaleBG[nIndex].gameObject.SetActive(value: false);
			return;
		}
		CleanRank[nIndex].alpha = 0f;
		CleanRank[nIndex].transform.localScale = new Vector3(11f, 11f, 1f);
		EffectRoot[nIndex].SetActive(value: false);
		CleanRankScaleBG[nIndex].gameObject.SetActive(value: true);
