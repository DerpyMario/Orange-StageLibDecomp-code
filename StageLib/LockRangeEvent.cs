...
public class LockRangeEvent : EventPointBase
{
	...
	public override void OnEvent(Transform TriggerTransform)
	{
		...
		if (bLockNet && sTriggerNetID == MonoBehaviourSingleton<GameServerService>.Instance.PlayerIdentify)
		{
			...
			StageUpdate.SyncStageObj(sSyncID, 0, sTriggerNetID + "," + position.x + "," + position.y + "," + position.z + "," + StageUpdate.GetLockRangeListStr(list), bNoCheckHost: true);
		}
		if (IsSelfLockRange())
		{
	...
	{
		StageUpdate.SyncStageObj(sSyncID, 2, GetBoolSaveStr(bCheck), bNoCheckHost: true);
	}
