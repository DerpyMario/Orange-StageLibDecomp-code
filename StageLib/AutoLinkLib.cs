...
public class AutoLinkLib
{
	...
	public static void FindAndLink(object target, Transform tTrans, FieldInfo fi)
	{
		...
		Transform transform = FindName(tTrans, fi.Name, bFirstLayer: false);
		if (transform != null)
		{
	...
	public static void FindAndLinkArray(object target, Transform tTrans, FieldInfo fi)
	{
		...
		Type type = Type.GetType($"{arg},{fi.FieldType.Assembly.GetName().Name}");
		...
		while (true)
		{
			transform = FindName(tTrans, fi.Name + num, bFirstLayer: false);
			if (transform == null)
			{
				break;
			}
