...
public class MapCollisionEvent : EventPointBase
{
	...
	public void CallOnByID(EventManager.StageEventCall tStageEventCall)
	{
		...
		case MapCollisionEnum.COLLISION_DESTRUCTIBLE:
			if (!(fParam0 <= 0f))
			{
				...
				{
					StageUpdate.SyncStageObj(sSyncID, 0, "," + fParam0.ToString("0.000"), bNoCheckHost: true);
				}
				else
				{
					StageUpdate.SyncStageObj(sSyncID, 0, stageObjBase.sNetSerialID + "," + fParam0.ToString("0.000"), bNoCheckHost: true);
				}
			}
			break;
		case MapCollisionEnum.COLLISION_KICKOBJ:
	...
	private void UpdateFlyObj(RunUpdateClass tRUC)
	{
		...
		if (tRUC.fParams[3] <= 0f)
		{
			...
			{
				array3[j].SwitchB2DInStageSceneObj(bSet: false);
			}
			if (listnParam0[2] != 0)
			{
	...
	private void MoveCheckHitBlock()
	{
		...
		for (int num2 = StageUpdate.runPlayers.Count - 1; num2 >= 0; num2--)
		{
			...
			{
				_ = StageUpdate.runPlayers[num2].Controller;
		...
		for (int i = 0; i < allController2Ds.Count; i++)
		{
			...
			{
			case 0:
				_ = controller2D._raycastOrigins;
				_ = Vector2.right * controller2D._verticalRaySpacing;
				break;
			case 1:
				_ = controller2D._raycastOrigins;
				_ = Vector2.right * controller2D._verticalRaySpacing;
			...
			case 2:
				_ = controller2D._raycastOrigins;
				_ = Vector2.up * controller2D._horizontalRaySpacing;
				break;
			case 3:
				_ = controller2D._raycastOrigins;
				_ = Vector2.up * controller2D._horizontalRaySpacing;
			...
			if ((bool)outhit)
			{
				...
				nNowStep++;
				Singleton<GenericEventManager>.Instance.NotifyEvent(EventManager.ID.CAMERA_SHAKE, 1f, p_param2: false);
	...
	public void InitCollisionData()
	{
		...
		case MapCollisionEnum.COLLISION_DESTRUCTIBLE:
		{
			...
			fTmpParam0 = fParam0;
			StageSceneObjParam[] componentsInChildren3 = base.transform.GetComponentsInChildren<StageSceneObjParam>(includeInactive: true);
			array = base.transform.GetComponentsInChildren<BoxCollider2D>(includeInactive: true);
			base.transform.GetComponentsInChildren<MeshRenderer>(includeInactive: true);
			array2 = base.transform.GetComponentsInChildren<Animation>(includeInactive: true);
	...
	private void HurtCPCheck(StageObjBase tShotSOB, Vector3? vDir = null)
	{
		...
		else if (fTmpParam0 * 0.3f >= fParam0 && fTmpParam0 * 0.3f < fTmpParam1)
		{
			...
			{
				obj.SwitchAnimatorInStageSceneObj(bSet: false);
	...
	private int Hurt(int dmg, Vector3 vDir, int nSkillID, StageObjBase tShotSOB)
	{
		...
		{
			StageUpdate.SyncStageObj(sSyncID, 0, tShotSOB.sNetSerialID + "," + fParam0.ToString("0.000"), bNoCheckHost: true);
		}
		return (int)fParam0;
	}
	...
	public override void LoadByString(string sLoad)
	{
		...
		case MapCollisionEnum.COLLISION_DESTRUCTIBLE:
			...
			if (array.Length > nIndex)
			{
				...
				{
					int.TryParse(array[nIndex++], out var result5);
			...
			if (array.Length > nIndex)
			{
				...
				{
					int.TryParse(array[nIndex++], out var result6);
					int.TryParse(array[nIndex++], out var result7);
			...
			if (array.Length > nIndex)
			{
				...
				{
					int.TryParse(array[nIndex++], out var result8);
			...
			if (array.Length > nIndex)
			{
				...
				{
					float.TryParse(array[nIndex++], out var result9);
					int.TryParse(array[nIndex++], out var result10);
			...
			if (array.Length > nIndex)
			{
				...
				{
					float.TryParse(array[nIndex++], out var result11);
					int.TryParse(array[nIndex++], out var result12);
		...
		case MapCollisionEnum.COLLISION_KICKOBJ:
			...
			if (array.Length > nIndex)
			{
				...
				{
					int.TryParse(array[nIndex++], out var result3);
			...
			if (array.Length > nIndex)
			{
				...
				{
					float.TryParse(array[nIndex++], out var result4);
