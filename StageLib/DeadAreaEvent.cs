...
public class DeadAreaEvent : EventPointBase
{
	...
	private IEnumerator NoNotifyPlayerCoroutine(OrangeCharacter tPlayer, bool bUseLastStandPt = false)
	{
		...
		tPlayer.DeadAreaLockEvt.CheckTargetToInvoke(p_param: true);
		...
		tPlayer.DeadAreaLockInputing = false;
		tPlayer.DeadAreaLockEvt.CheckTargetToInvoke(p_param: false);
	...
	public void OnEventPlayer(OrangeCharacter tPlayer)
	{
		...
		if (bMoveByNotify)
		{
			...
			{
				stageUpdate.tDictDeadAreaEventCoroutine.Add(tPlayer.GetInstanceID(), stageUpdate.StartCoroutine(NoNotifyPlayerCoroutine(tPlayer, bUseLastStandPt: true)));
			}
			else
			{
				listEventPlayers.Remove(tPlayer.sPlayerID);
