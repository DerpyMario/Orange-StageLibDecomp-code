...
public class StageEnemy : StageSLBase
{
	...
	private void CallEnemySpawn()
	{
		...
		if (!(enemySpawn != null))
		{
			...
			if (enemySpawn != null)
			{
				...
				enemySpawn.SetActive(isActive: true);
	...
	public void SpawnGG()
	{
		if (MonoBehaviourSingleton<OrangeSceneManager>.Instance.MainCamera == null || StageResManager.GetStageUpdate().IsEnd)
		{
			return;
		}
		if (!bBornAtStart)
		...
		{
			StageUpdate.SyncStageObj(sSyncID, 3, "", bNoCheckHost: true);
			return;
		}
		fWaitTime = 0f;
		bBornAtStart = false;
