...
public class StageResManager : MonoBehaviour
{
	...
	public static T GetObjFromPool<T>() where T : new()
	{
		...
		if (StagePoolDic.TryGetValue(typeFromHandle, out var value))
		{
			if (value.Count > 0)
			{
				T result = (T)value[0];
	...
	public static void InitStageBattleUI(Action loadendcb)
	{
		...
		if (MonoBehaviourSingleton<OrangeBattleServerManager>.Instance.IsPvp && ManagedSingleton<OrangeDataManager>.Instance.STAGE_TABLE_DICT.TryGetValue(ManagedSingleton<StageHelper>.Instance.nLastStageID, out var value) && value.n_MAIN == 90001 && value.n_SUB == 1)
		{
			loadCallBackObj = new StageUpdate.LoadCallBackObj();
			loadCallBackObj.lcb = delegate(StageUpdate.LoadCallBackObj tObj, UnityEngine.Object asset)
			{
	...
	{
		if (!ManagedSingleton<OrangeDataManager>.Instance.SKILL_TABLE_DICT.TryGetValue(nSkillID, out var tSKILL_TABLE) || tSKILL_TABLE.s_MODEL == "DUMMY" || tSKILL_TABLE.s_MODEL == "null")
		{
			return;
		}
		StageUpdate.LoadCallBackObj loadCallBackObj = new StageUpdate.LoadCallBackObj();
	...
	{
		if (ManagedSingleton<OrangeDataManager>.Instance.SKILL_TABLE_DICT.TryGetValue(nID, out var value))
		{
	...
	{
		if (ManagedSingleton<OrangeDataManager>.Instance.CONDITION_TABLE_DICT.TryGetValue(nBuffID, out var value))
		{
	...
	public static bool LoadEnemy(MOB_TABLE tMob)
	{
		...
		{
			if (ManagedSingleton<OrangeDataManager>.Instance.SKILL_TABLE_DICT.TryGetValue(array[i], out var value))
			{
				list.Add(value);
			}
		}
	...
	public static ObjInfoBar CreateInfoBar(Transform tFollow, BoxCollider2D tB2d, bool bCaluHeigh = false, float fadd = 0.5f)
	{
		...
		float y = 0f;
		Transform transform = OrangeBattleUtility.FindChildRecursive(tFollow, "InfoBar", equal: true);
		if (transform == null)
		{
			if (tB2d != null)
			{
		...
		poolObj.transform.SetParent(transform, worldPositionStays: false);
	...
	{
		ObjInfoBar objInfoBar = CreateInfoBar(tPlayer.transform, tPlayer.Controller.Collider2D, bCaluHeigh: false, 1.25f);
	...
	public static EnemyControllerBase CreateEnemyByMob(MOB_TABLE tMOB_TABLE)
	{
		...
		{
			BattleInfoUI.Instance.InitBar(poolObj, tMOB_TABLE.n_HP_STEP, tMOB_TABLE.n_HP, tMOB_TABLE.n_HP, tMOB_TABLE.s_ICON, bDeadHiden: false);
			poolObj.selfBuffManager.ClearBuff();
		}
		else
		{
			ObjInfoBar objInfoBar = CreateInfoBar(poolObj.transform, component, bCaluHeigh: true);
	...
	{
		if (!ManagedSingleton<OrangeDataManager>.Instance.WEAPON_TABLE_DICT.TryGetValue(nWeaponID, out var value))
		{
			return;
		}
		string text = "";
		if (ManagedSingleton<OrangeDataManager>.Instance.SKILL_TABLE_DICT.TryGetValue(value.n_SKILL, out var value2))
		{
	...
	{
		if (ManagedSingleton<OrangeDataManager>.Instance.CHARACTER_TABLE_DICT.TryGetValue(nCharacterID, out var value))
		{
			StageUpdate.LoadCallBackObj loadCallBackObj = new StageUpdate.LoadCallBackObj();
			AddCb(loadCallBackObj);
			HumanBase.LoadMotion(loadCallBackObj, value.s_ANIMATOR, WeaponType.Dummy);
			if (ManagedSingleton<OrangeDataManager>.Instance.WEAPON_TABLE_DICT.TryGetValue(nWeaponID1, out var value2))
			{
				StageUpdate.LoadCallBackObj loadCallBackObj2 = new StageUpdate.LoadCallBackObj();
				AddCb(loadCallBackObj2);
	...
	public static RefPassiveskill LoadAllPassSkill(StageHelper.StageCharacterStruct tStageCharacterStruct, MemberInfo tMemberInfo = null)
	{
		...
		if (tMemberInfo != null)
		{
			...
			foreach (NetChipInfo totalChip in tMemberInfo.netSealBattleSettingInfo.TotalChipList)
			{
				...
				{
					refPassiveskill.AddPassivesSkill(totalChip, 4, 1, bCheckUse: true);
				}
				else if (totalChip.ChipID == tMemberInfo.netSealBattleSettingInfo.SubWeaponInfo.Chip)
				{
					refPassiveskill.AddPassivesSkill(totalChip, 8, 1, bCheckUse: true);
				}
			}
		}
		else
		{
			if (ManagedSingleton<PlayerNetManager>.Instance.dicWeapon.TryGetValue(tStageCharacterStruct.MainWeaponID, out value))
			{
				...
				{
					refPassiveskill.AddPassivesSkill(value2.netChipInfo, 4, 1, bCheckUse: true);
			...
			if (ManagedSingleton<PlayerNetManager>.Instance.dicWeapon.TryGetValue(tStageCharacterStruct.SubWeaponID, out value))
			{
				...
				{
					refPassiveskill.AddPassivesSkill(value3.netChipInfo, 8, 1, bCheckUse: true);
	...
	public static T[] GetAllTypeOfObjs<T>()
	{
		...
		for (int i = 0; i < rootGameObjects.Length; i++)
		{
			list.AddRange(rootGameObjects[i].GetComponentsInChildren<T>(includeInactive: true));
		}
		return list.ToArray();
	}
