...
public class StageRideableObj : StageSLBase
{
	...
	public override void LoadByString(string sLoad)
	{
		...
		if (ManagedSingleton<OrangeDataManager>.Instance.VEHICLE_TABLE_DICT.TryGetValue(nRideID, out var value))
		{
			...
			StageUpdate.LoadCallBackObj loadCallBackObj = new StageUpdate.LoadCallBackObj
			{
				lcb = delegate(StageUpdate.LoadCallBackObj tLCB, Object tObj)
				{
					...
					((GameObject)vehicleInstance).SetActive(value: false);
				}
			};
			...
			if (ManagedSingleton<OrangeDataManager>.Instance.SKILL_TABLE_DICT.TryGetValue(value.n_SKILL_1, out var value2))
			{
