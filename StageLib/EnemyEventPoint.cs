...
public class EnemyEventPoint : EventPointBase
{
	...
	public IEnumerator ReStartEvent(bool bNextGroup = false)
	{
		...
		else
		{
			...
			{
				StageUpdate.SyncStageObj(sSyncID, 0, _nowEventGroup + "," + _sTriggerNetID, bNoCheckHost: true);
		...
		_bStartEvent = false;
		StageUpdate.SyncStageObj(sSyncID, 3, nEndEventID.ToString(), bNoCheckHost: true);
		TriggerUnLockRange();
		TriggerEndEventCall();
		...
		else
		{
			...
			if (bCanCallReRun && !bCheck)
			{
				...
				else if (_nowStartIndex >= _nCount && !_bRebornFlag)
				{
					...
					StageUpdate.SyncStageObj(sSyncID, 8, _nowEventGroup + "," + _nowRebornIndex + "," + text, bNoCheckHost: true);
				}
			}
			OnEvent(tTransform);
		}
	...
	public override void OnLateUpdate()
	{
		...
		while (_nowStartIndex < _nCount)
		{
			...
			else if (SpawnEnemy(enemySpawnData, text, _nowStartIndex, bNeedAddEnemyCount: false, bNeedCheckBornInB2D: true))
			{
				flag = true;
			}
			_nowStartIndex++;
		...
		if (flag)
		{
			StageUpdate.SyncStageObj(sSyncID, 1, _nowEventGroup + "," + (_nowStartIndex - 1), bNoCheckHost: true);
		}
		...
		if (_bRebornFlag)
		{
			...
			if (flag)
			{
				StageUpdate.SyncStageObj(sSyncID, 6, _nowEventGroup + "," + (_nowRebornIndex - 1), bNoCheckHost: true);
			}
		...
		{
			_bStartEvent = false;
			StartCoroutine(ReStartEvent(bNextGroup: true));
		}
	}
	...
	public void HurtCB(StageObjBase tSOB)
	{
		...
		for (int num = _inGameEnemy.Count - 1; num >= 0; num--)
		{
			if (_inGameEnemy[num].targetEnemy != null && _inGameEnemy[num].sNetSerialID == tSOB.sNetSerialID)
			{
				...
				{
					StageUpdate.SyncStageObj(sSyncID, 2, _nowEventGroup + "," + _inGameEnemy[num].sNetSerialID, bNoCheckHost: true);
				}
				SetEnemyDead(num);
				break;
			}
	...
	public override void SyncNowStatus()
	{
		if (!bCheck)
		{
			...
			StageUpdate.SyncStageObj(sSyncID, 5, _sTriggerNetID + "," + _nowEventGroup + "," + _nowStartIndex + "," + _nowEventTime + "," + text, bNoCheckHost: true);
		}
		else
		{
			StageUpdate.SyncStageObj(sSyncID, 7, "", bNoCheckHost: true);
		}
	}
	...
	public override void OnSyncStageObj(string sIDKey, int nKey1, string smsg)
	{
		...
		case 1:
		{
			...
			while (_nowStartIndex < num3 + 1)
			{
				...
				{
					SpawnEnemy(tEnemySpawnData, text, _nowStartIndex, bNeedAddEnemyCount: false);
				}
				_nowStartIndex++;
			}
			break;
		...
		case 5:
		{
			...
			while (num5 < array3.Length)
			{
				...
				if (!SpawnEnemy(enemySpawnData2, sTmpSerialID, num8, bNeedAddEnemyCount: true, bNeedCheckBornInB2D: false, value))
				{
					return;
				}
				enemySpawnData2.targetEnemy.Hp = num9;
	...
	private void TriggerEndEventCall()
	{
		...
		case EventManager.ID.CAMERA_SHAKE:
			Singleton<GenericEventManager>.Instance.NotifyEvent(iD, (float)nEndParam1, p_param2: false);
			break;
			...
			for (int num = StageUpdate.listAllEvent.Count - 1; num >= 0; num--)
			{
				if (StageUpdate.listAllEvent[num].nSetID == nUnlockRangeID)
				{
					...
					if (lockRangeEvent != null)
					{
						...
						if (StageUpdate.CheckLockRangeList(StageUpdate.listAllEvent[num].sSyncID, sPlayerID, bCheckIsLast: true))
						{
		...
		if (enemyControllerBase != null)
		{
			...
			if (bNeedCheckBornInB2D && bCheckBornInB2D)
			{
				...
				if (tOutList.Count != 0 && ((int)enemyControllerBase.Controller.collisionMask | (1 << ManagedSingleton<OrangeLayerManager>.Instance.BlockLayer)) != 0)
				{
					...
					enemyControllerBase.OnlySwitchMaterial(bEnable: true);
				}
				else
				{
