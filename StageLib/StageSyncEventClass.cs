...
public class StageSyncEventClass
{
	...
	public void STAGE_PLAYER_REBORN(string sMsg)
	{
		...
		if (splitstrs[1] == "0")
		{
			...
			{
				StageUpdate.SyncStageObj(3, 3, splitstrs[0] + ",1," + splitstrs[2], bNoCheckHost: true, bNoCheckReConnect: true);
			}
		}
		else if (splitstrs[1] == "1")
		{
			...
			{
				StageUpdate.SyncStageObj(3, 3, MonoBehaviourSingleton<GameServerService>.Instance.PlayerIdentify + ",2," + splitstrs[2] + "," + ManagedSingleton<InputStorage>.Instance.GetInputRecordNO(MonoBehaviourSingleton<GameServerService>.Instance.PlayerIdentify), bNoCheckHost: true);
			...
			else
			{
				...
				if (splitstrs[2] == "1")
				{
					...
					{
						Singleton<GenericEventManager>.Instance.NotifyEvent<string, bool, float, float, bool?>(EventManager.ID.STAGE_CONTINUE_PLATER, MonoBehaviourSingleton<GameServerService>.Instance.PlayerIdentify, p_param2: false, 0f, 0f, null);
						BattleInfoUI.Instance.SwitchOptionBtn(bShow: true, 1);
		...
		else
		{
			...
			if (splitstrs.Length >= 5)
			{
				...
				if (splitstrs.Length >= 6)
				{
					if (splitstrs[6] == "1")
					{
						Singleton<GenericEventManager>.Instance.NotifyEvent(EventManager.ID.STAGE_CONTINUE_PLATER, splitstrs[0], p_param2: true, float.Parse(splitstrs[3]), float.Parse(splitstrs[4]), (bool?)true);
					}
					else
					{
						Singleton<GenericEventManager>.Instance.NotifyEvent(EventManager.ID.STAGE_CONTINUE_PLATER, splitstrs[0], p_param2: true, float.Parse(splitstrs[3]), float.Parse(splitstrs[4]), (bool?)false);
					}
				}
				else
				{
					Singleton<GenericEventManager>.Instance.NotifyEvent<string, bool, float, float, bool?>(EventManager.ID.STAGE_CONTINUE_PLATER, splitstrs[0], p_param2: true, float.Parse(splitstrs[3]), float.Parse(splitstrs[4]), null);
				}
			}
			else
			{
				ManagedSingleton<InputStorage>.Instance.SetInputRecordNO(splitstrs[0], int.Parse(splitstrs[3]));
				Singleton<GenericEventManager>.Instance.NotifyEvent<string, bool, float, float, bool?>(EventManager.ID.STAGE_CONTINUE_PLATER, splitstrs[0], p_param2: false, 0f, 0f, null);
			}
		}
	}
				...
				if (StageUpdate.runPlayers[num2].sPlayerID == syncOCData.sPlayerID)
				{
					if (syncOCData.nCharID == StageUpdate.runPlayers[num2].CharacterID)
					{
						...
						if ((int)StageUpdate.runPlayers[num2].Hp <= 0)
						{
							...
							{
								Singleton<GenericEventManager>.Instance.NotifyEvent<string, bool, float, float, bool?>(EventManager.ID.STAGE_CONTINUE_PLATER, syncOCData.sPlayerID, p_param2: false, 0f, 0f, null);
								StageUpdate.runPlayers[num2].Hp = syncOCData.nHP;
								StageUpdate.runPlayers[num2].selfBuffManager.SyncByNetBuff(syncOCData.listSyncBuff, bIgnoreWaitNetSync: true);
							}
						}
						else
						{
					...
					{
						StageUpdate.runPlayers[num2].PlayerDead(bJustRemove: true);
					}
					break;
				}
			}
	...
	private void STAGE_SYNC_OBJ_ENEMY_DATA(int nIndex, string[] splitstrs)
	{
		...
		while (nIndex < splitstrs.Length)
		{
			...
			if (!flag2)
			{
				...
				enemyControllerBase.SetActive(isActive: true);
			}
		}
	...
	private IEnumerator NotifyPlayerNetCtrlOnCoroutine(string sPlayerID)
	{
		...
		{
			Singleton<GenericEventManager>.Instance.NotifyEvent(EventManager.ID.PLAYERBUILD_PLAYER_NETCTRLON, sPlayerID, p_param2: false);
	...
	public void STAGE_SYNC_PLAYEROUTGAME(string sMsg)
	{
		...
		if (MonoBehaviourSingleton<OrangeBattleServerManager>.Instance.GetPlayingPlayerCount(bIgnoreSelf: true) == 0)
		{
	...
	private IEnumerator WaitReMovePlayerCoroutine(string sPlayerID)
	{
		...
		while (tStageUpdate.nRunStageCtrlCount > 0)
		{
			...
			if (tOC.IsDead())
			{
				Singleton<GenericEventManager>.Instance.NotifyEvent<string, bool, float, float, bool?>(EventManager.ID.STAGE_CONTINUE_PLATER, tOC.sNetSerialID, p_param2: false, 0f, 0f, null);
			}
		}
		Singleton<GenericEventManager>.Instance.NotifyEvent(EventManager.ID.STAGE_PLAYER_DESTROY_ED, tOC, p_param2: true);
		tOC.SetActiveFalse();
	}
	...
	private IEnumerator WaitAllPlayerOnAndSendReConnectData(string smsg)
	{
		...
		if (tStageUpdate.IsEnd)
		{
			if (!MonoBehaviourSingleton<OrangeBattleServerManager>.Instance.IsPvp)
			{
				...
				StageUpdate.SyncStageObj(3, 13, MonoBehaviourSingleton<GameServerService>.Instance.PlayerIdentify + "," + (int)ManagedSingleton<StageHelper>.Instance.eLastStageResult, bNoCheckHost: true, bNoCheckReConnect: true);
			}
			yield break;
		}
		while (true)
		...
		while (true)
		{
			...
			if (fSendWait >= 0.8f)
			{
				StageUpdate.SyncStageObj(3, 15, MonoBehaviourSingleton<GameServerService>.Instance.PlayerIdentify, bNoCheckHost: true, bNoCheckReConnect: true);
				fSendWait = 0f;
			}
			yield return CoroutineDefine._waitForEndOfFrame;
		}
