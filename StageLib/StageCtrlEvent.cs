...
public class StageCtrlEvent : EventPointBase
{
	...
	public override void OnLateUpdate()
	{
		...
		while (nStartIndex < StageCtrlInsTructions.Count)
		{
			...
			{
				ManagedSingleton<OrangeDataManager>.Instance.STAGE_TABLE_DICT.TryGetValue(ManagedSingleton<StageHelper>.Instance.nLastStageID, out var value);
				bool flag = false;
				...
				else if (value != null)
				{
					...
					{
						if (ManagedSingleton<OrangeDataManager>.Instance.STAGE_TABLE_DICT.TryGetValue(enumerator.Current.Value.netStageInfo.StageID, out var value2) && value2.n_MAIN == value.n_MAIN)
						{
							flag = true;
							break;
						}
		...
		if (text.Length > 0)
		{
			StageUpdate.SyncStageObj(sSyncID, 3, text + "," + fLeftTime.ToString("0.000"), bNoCheckHost: true);
		}
	...
	private bool CTRL_MOVE_ATKBTN(StageCtrlInsTruction tStageCtrlInsTruction)
	{
		...
		{
			StageUpdate.GetObjBySCEID(num, out var _, out var tOC);
			if (tOC != null)
			{
	...
	private bool CTRL_PLAYER_TELEPORT(StageCtrlInsTruction tStageCtrlInsTruction)
	{
		if (tObj != null)
		{
			...
			notifyCallBack.cbparam = ContinuePlay;
			notifyCallBack.nParam0 = ++nStartIndex;
			StartCoroutine(WaitTeleportCoroutine(MonoBehaviourSingleton<GameServerService>.Instance.PlayerIdentify, localPosition, fWaitTime, notifyCallBack, bShowEffect: false, bSetLookDir, bLookFront, bTeleportWaitTime));
		}
		return CheckLockReturn();
	}
				...
				if (uI_Parent != null)
				{
					...
					if (!(sLockName == ""))
					{
						...
						if (transform != null)
						{
							...
							RectTransformUtility.ScreenPointToLocalPointInRectangle(component.GetComponent<RectTransform>(), screenPoint, component.worldCamera, out var localPoint);
	...
	private bool CTRL_SET_OBJ_ACT(StageCtrlInsTruction tStageCtrlInsTruction)
	{
		...
		StageUpdate.GetObjBySCEID(nSCEID, out var tECB, out var tOC);
		if (!flag)
		{
			if (tECB != null)
			{
			...
			else if (tOC != null)
			{
				...
				else if (tStageCtrlInsTruction.sMsg == "PlayCharge1")
				{
					...
					if ((bool)component)
					{
						component.PlayCharge(bLv1: true, bLv2: false);
					}
				}
				else if (tStageCtrlInsTruction.sMsg == "PlayCharge2")
				{
					...
					if ((bool)component2)
					{
						component2.PlayCharge(bLv1: false, bLv2: true);
					}
				}
				...
				else if (tStageCtrlInsTruction.sMsg.StartsWith("UnRide"))
				{
					...
					if (tOC.refRideBaseObj != null)
					{
						if (text == "1")
						{
							tOC.refRideBaseObj.UnRide(bDisable: true);
						}
						else
						{
							tOC.refRideBaseObj.UnRide(bDisable: false);
						}
					}
				}
				else
				{
					...
					if ((bool)component3)
					{
						component3.PlayCharge(bLv1: false, bLv2: false);
					}
				}
			}
		}
		else
		{
			...
			if (tOC != null)
			{
				...
				if ((bool)component5)
				{
					component5.PlayCharge(bLv1: false, bLv2: false);
				}
				return CheckLockReturn();
			}
		}
	...
	private bool CTRL_SET_DEAD_CALLEVENT(StageCtrlInsTruction tStageCtrlInsTruction)
	{
		...
		StageUpdate.GetObjBySCEID(nSCEID, out var tECB, out var _);
		if (tECB != null)
		{
	...
	private bool CTRL_MOVE_OBJ_TO_OBJ(StageCtrlInsTruction tStageCtrlInsTruction)
	{
		...
		StageUpdate.GetObjBySCEID(nSCEID, out var tECB, out var tOC);
		StageUpdate.GetObjBySCEID(nSCEID2, out var tECB2, out var tOC2);
	...
	private bool CTRL_SET_OBJ_DIR(StageCtrlInsTruction tStageCtrlInsTruction)
	{
		...
		else if (tECB != null)
		{
			if (tECB2 != null)
			{
				...
				{
					tECB.SetPositionAndRotation(tECB.transform.position, bBack: true);
				}
				else
				{
					tECB.SetPositionAndRotation(tECB.transform.position, bBack: false);
				}
			}
			else if (tOC2 != null)
			{
	...
	private bool CTRL_OBJ_ADDBUFF(StageCtrlInsTruction tStageCtrlInsTruction)
	{
		...
		if (tECB != null)
		{
			...
			{
				tECB.selfBuffManager.RemoveBuff(num, bJustRemove: false);
			}
		}
		else if (tOC != null)
		{
			...
			{
				tOC.selfBuffManager.RemoveBuff(num, bJustRemove: false);
			}
		}
		return false;
	}
	...
	private bool CTRL_PLAYER_TELEPORTEFFECT(StageCtrlInsTruction tStageCtrlInsTruction)
	{
		if (tObj != null)
		{
			...
			notifyCallBack.cbparam = ContinuePlay;
			notifyCallBack.nParam0 = ++nStartIndex;
			StartCoroutine(WaitTeleportCoroutine(MonoBehaviourSingleton<GameServerService>.Instance.PlayerIdentify, localPosition, fWaitTime, notifyCallBack, bShowEffect: true, bSetLookDir, bLookFront, bTeleportWaitTime));
		}
		return CheckLockReturn();
	}
	...
	private bool CTRL_SHOW_BOSSHPBAR(StageCtrlInsTruction tStageCtrlInsTruction)
	{
		if (tStageCtrlInsTruction.nParam1 == 1f)
		{
			if (BattleInfoUI.Instance != null)
			{
				BattleInfoUI.Instance.SetHiddenBossBar(bHidden: false);
			}
		}
		else
		{
			BattleInfoUI.Instance.SetHiddenBossBar(bHidden: true);
		}
		return false;
	}
	...
	private IEnumerator WaitTeleportCoroutine(string sPlayerID, Vector3 tPos, float fWaitTime, NotifyCallBack tNCB, bool bShowEffect = false, bool bSetLookDir = false, bool bLookFront = false, bool bTeleportWaitTime = false)
	{
		...
		{
			StageUpdate.SyncStageObj(sSyncID, 10, tOC3.sNetSerialID + "," + tPos.x + "," + tPos.y, bNoCheckHost: true);
		}
		if (bShowEffect)
		{
	...
	{
		if (tRUC.oParams[0] != null && tRUC.oParams[4] is NotifyCallBack notifyCallBack)
		{
			notifyCallBack.CallCB();
		}
	}
	...
	private void WaitObjMoveToPos(RunUpdateClass tRUC)
	{
		if (tRUC.oParams[0] != null)
		{
			...
			else
			{
				...
				{
					tRUC.bIsEnd = true;
					if (tRUC.oParams[4] is NotifyCallBack notifyCallBack)
					{
						notifyCallBack.CallCB();
					}
				}
		...
		else
		{
			...
			{
				tRUC.bIsEnd = true;
				if (tRUC.oParams[4] is NotifyCallBack notifyCallBack2)
				{
	...
	{
		StageUpdate.SyncStageObj(sSyncID, 6, tNCB.nParam0.ToString(), bNoCheckHost: true);
		ContinuePlay(tNCB);
	}
	...
	private IEnumerator NetWaitContinuePlay(int nWaitIndex)
	{
		...
		{
			_ = StageCtrlInsTructions[nStartIndex - 1].tStageCtrl;
	...
	public override void SyncNowStatus()
	{
		if (!bCheck)
		{
			...
			StageUpdate.SyncStageObj(sSyncID, 8, smsg, bNoCheckHost: true);
		}
	}
	...
	public override void OnSyncStageObj(string sIDKey, int nKey1, string smsg)
	{
		...
		case 3:
		{
			...
			{
				if (int.TryParse(array2[i], out var result) && result > nStartIndex)
				{
		...
		case 6:
		{
			if (int.TryParse(smsg, out var result2) && (result2 > nStartIndex || (result2 == nStartIndex && bLockWait)))
			{
