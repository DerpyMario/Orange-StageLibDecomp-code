...
public class OrangeTableHelper : ManagedSingleton<OrangeTableHelper>
{
	...
	{
		ManagedSingleton<OrangeDataManager>.Instance.CHARACTER_TABLE_DICT.TryGetValue(characterId, out var value);
		return value;
	}
	...
	{
		if (p_dict.TryGetValue(p_key, out var value))
		{
			p_list.Add(value);
		}
	}
	...
	{
		if (ManagedSingleton<OrangeDataManager>.Instance.EXP_TABLE_DICT.TryGetValue(p_level, out var value))
		{
	...
	public bool IsStageVaild(int p_stageId)
	{
		...
		if (GetStage(p_stageId, out var stage) && dicStageVaild.TryGetValue(p_stageId, out var value))
		{
	...
	{
		if (cacheStageType != StageType.None && cacheStageType == p_type)
		{
	...
	{
		if (ManagedSingleton<ExtendDataHelper>.Instance.ITEM_TABLE_DICT.TryGetValue(p_itemId, out var value))
		{
			return value.n_MAX;
		}
		return 0;
	...
	public bool IsOpeningDate(string p_beginTime, string p_endTime, long p_now)
	{
		...
		{
			UnityEngine.Debug.Log($"p_beginTime={p_beginTime}, p_endTime={p_endTime} msg={ex.Message}");
		}
		return false;
	}
	...
	public bool IsTimeAfterDate(string p_checkDate, long p_checkTime)
	{
		...
		{
			UnityEngine.Debug.Log($"p_checkDate={p_checkDate}, msg={ex.Message}");
		}
		return false;
	}
	...
	{
		return "ui_iconsource_card_ball_s_" + ((CardColorType)typ/*cast due to .constrained prefix*/).ToString().ToLower();
	}
