...
public class CameraEffectManager : MonoBehaviour
{
	...
	private void CameraEffectControl(bool isEnable, string effectName)
	{
		...
		if (effectList.TryGetValue(effectName, out var value))
		{
			...
			if (value.controller != null)
			{
				if (isEnable)
				{
					...
					{
						value.controller.gameObject.SetActive(value: true);
				...
				else
				{
					...
					value.controller.gameObject.SetActive(value: false);
				}
			}
		}
		else
		...
		Debug.Log($"Camera effect event catched : {isEnable} {effectName}");
	}
	...
	private void CameraEffectLoadDone(GameObject effectObject)
	{
		...
		if (effectList[effectObject.name].isEnable)
		{
			...
			{
				effectList[effectObject.name].controller.gameObject.SetActive(value: true);
		...
		else
		{
			...
			effectList[effectObject.name].controller.gameObject.SetActive(value: false);
		}
	}
}
