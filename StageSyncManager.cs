...
public class StageSyncManager : MonoBehaviourSingleton<StageSyncManager>
{
	...
	public float HostAvgDelayTime => fAvgDelayTime;
	...
	private void OnApplicationPause(bool pause)
	{
		if (StageUpdate.nReConnectMode == 0)
		{
			...
			if (StageUpdate.gbGeneratePvePlayer)
			{
				...
				StageUpdate.SyncStageObj(3, 18, MonoBehaviourSingleton<GameServerService>.Instance.PlayerIdentify + ",0", bNoCheckHost: true);
	...
	private void ObjCtrl(GameObject tObj, StageCtrlInsTruction tSCE)
	{
		...
		case 18:
		{
			...
			{
				StageUpdate.runBulletSets[num2].PauseUseSE(bPause: true);
			}
			break;
		}
		case 19:
			...
			if (!MonoBehaviourSingleton<OrangeGameManager>.Instance.IsGamePause)
			{
				...
				{
					StageUpdate.runBulletSets[num].PauseUseSE(bPause: false);
				}
			}
			break;
		}
	...
	public void OnNTSyncStageObj(object obj)
	{
		...
		case 5:
			...
			case 0:
				...
				break;
			case 1:
				MonoBehaviourSingleton<OrangeBattleServerManager>.Instance.SetPlayerPause(splitstrs[0], bIsPause: true);
				break;
			case 2:
				MonoBehaviourSingleton<OrangeBattleServerManager>.Instance.SetPlayerPause(splitstrs[0], bIsPause: false);
				break;
			case 3:
			...
			if (orangeCharacter != null)
			{
				...
				if (flag)
				{
					...
					if (value.n_CONDITION_ID != 0 && value.n_CONDITION_RATE >= UnityEngine.Random.Range(0, 10000))
					{
						...
						{
							orangeCharacter.selfBuffManager.AddBuff(value.n_CONDITION_ID, 0, 0, value.n_ID, bIsNetAdd: false, orangeCharacter.sNetSerialID, 4);
						}
						else
						{
							orangeCharacter.selfBuffManager.AddBuff(value.n_CONDITION_ID, 0, 0, value.n_ID, bIsNetAdd: false, orangeCharacter.sNetSerialID);
						}
					}
				}
			...
			else if (rideArmorController != null)
			{
				...
				if (flag)
				{
					...
					if (value.n_CONDITION_ID != 0 && value.n_CONDITION_RATE >= UnityEngine.Random.Range(0, 10000))
					{
						...
						{
							rideArmorController.selfBuffManager.AddBuff(value.n_CONDITION_ID, 0, 0, value.n_ID, bIsNetAdd: false, rideArmorController.sNetSerialID, 4);
						}
						else
						{
							rideArmorController.selfBuffManager.AddBuff(value.n_CONDITION_ID, 0, 0, value.n_ID, bIsNetAdd: false, rideArmorController.sNetSerialID);
						}
					}
				}
	...
	{
		if (ManagedSingleton<PlayerNetManager>.Instance.dicFinalStrike.TryGetValue(nFSID, out var value))
		{
