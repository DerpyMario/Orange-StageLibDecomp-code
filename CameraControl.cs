...
public class CameraControl : MonoBehaviour
{
	...
	private struct FocusArea
	{
		...
		public void Update(Vector3 focusCenter, LockRange targetLockRange)
		{
			...
			else if (_right - _left > targetLockRange.MaxX - targetLockRange.MinX)
			{
				...
				UpdateSmooth(ref _left, ref _right, dis, bCheck: true, num);
			}
			else if (_left < targetLockRange.MinX)
			{
				dis = targetLockRange.MinX - _left;
				UpdateSmooth(ref _left, ref _right, dis, bCheck: false, num);
			}
			else if (_right > targetLockRange.MaxX)
			{
				dis = _right - targetLockRange.MaxX;
				UpdateSmooth(ref _left, ref _right, 0f - dis, bCheck: true, num);
			}
			shiftY = 0f;
			...
			else if (_top - _bottom > targetLockRange.MaxY - targetLockRange.MinY)
			{
				...
				UpdateSmooth(ref _top, ref _bottom, dis, bCheck: true, num);
			}
			else if (_bottom < targetLockRange.MinY)
			{
				dis = targetLockRange.MinY - _bottom;
				UpdateSmooth(ref _top, ref _bottom, dis, bCheck: false, num);
			}
			else if (_top > targetLockRange.MaxY)
			{
				dis = _top - targetLockRange.MaxY;
				UpdateSmooth(ref _top, ref _bottom, 0f - dis, bCheck: true, num);
			}
			Center.x = (_left + _right) / 2f;
			Center.y = (_top + _bottom) / 2f;
		}
	...
	private float CameraMaxRangeX => CurrentLockRange.MaxX + ManagedSingleton<StageHelper>.Instance.fCameraWHalf;

	private float CameraMinRangeX => CurrentLockRange.MinX - ManagedSingleton<StageHelper>.Instance.fCameraWHalf;

	private float CameraMaxRangeY => CurrentLockRange.MaxY + ManagedSingleton<StageHelper>.Instance.fCameraHHalf;

	private float CameraMinRangeY => CurrentLockRange.MinY - ManagedSingleton<StageHelper>.Instance.fCameraHHalf;
	...
	public static float fMaxEdgeDis => Mathf.Min(5f, ManagedSingleton<StageHelper>.Instance.fCameraWHalf);
