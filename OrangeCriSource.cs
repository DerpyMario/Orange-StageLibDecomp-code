...
public class OrangeCriSource : MonoBehaviour
{
	...
	public bool InView => _currentDis < 10f;
	...
	private IEnumerator DelayForcePlay(string s_acb, string cuename, float delay)
	{
		yield return new WaitForSeconds(delay);
		IsVisiable = true;
		CheckAndPlay(s_acb, cuename, forceplay: true);
	}
	...
	public CriAtomExPlayback ForcePlaySE(string s_acb, string cuename, float delay = 0f)
	{
		...
		f_vol = 1f;
		_currentDis = 0f;
		return CheckAndPlay(s_acb, cuename, forceplay: true);
	}
	...
	{
		string key = acb + "," + cue;
		if (loopSEs.TryGetValue(key, out var value))
		{
			value.Reset();
			loopSEs.Remove(key);
		}
