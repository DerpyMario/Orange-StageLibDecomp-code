...
internal class OrangeBattleServerManager : MonoBehaviourSingleton<OrangeBattleServerManager>
{
	...
	public bool IsMultiply => goStageData != null;
	...
	public float GetCurrentFatiguePower()
	{
		...
		_ = FATIGUED_ABILITY_CORRECTION[Fatigued];
	...
	public void SpawnPlayer(MemberInfo tMemberInfo, bool isTeamMember, Vector3 tPos, Vector3 tOffsetPos, bool bLookBack = false, Action<OrangeCharacter> endcb = null)
	{
		...
		foreach (NetChipInfo totalChip in tMemberInfo.netSealBattleSettingInfo.TotalChipList)
		{
			...
			{
				chipStatus += ManagedSingleton<StatusHelper>.Instance.GetChipStatusX(chipInfo, 0, bAddStar: false, bAddAnalyse: false, null, true, tMemberInfo.netSealBattleSettingInfo.PlayerInfo.Exp);
			}
			else
			{
				chipStatus += ManagedSingleton<StatusHelper>.Instance.GetChipStatusX(chipInfo, 0, bAddStar: false, bAddAnalyse: false, null, false, tMemberInfo.netSealBattleSettingInfo.PlayerInfo.Exp);
			}
		}
		...
		if (IsPvp)
		{
			playerBuilder.SetPBP.mainWStatus = ManagedSingleton<StatusHelper>.Instance.GetWeaponStatusX(weaponInfo, -weaponInfo.netInfo.Exp, bAddStar: false, null, null, tMemberInfo.netSealBattleSettingInfo.PlayerInfo.Exp);
		}
		else
		{
			...
			playerBuilder.SetPBP.mainWStatus = ManagedSingleton<StatusHelper>.Instance.GetWeaponStatusX(weaponInfo, 0, bAddStar: false, null, null, tMemberInfo.netSealBattleSettingInfo.PlayerInfo.Exp);
		}
		weaponInfo = new WeaponInfo();
		weaponInfo.netInfo = tMemberInfo.netSealBattleSettingInfo.SubWeaponInfo;
		if (IsPvp)
		{
			playerBuilder.SetPBP.subWStatus = ManagedSingleton<StatusHelper>.Instance.GetWeaponStatusX(weaponInfo, -weaponInfo.netInfo.Exp, bAddStar: false, null, null, tMemberInfo.netSealBattleSettingInfo.PlayerInfo.Exp);
		}
		else
		{
			...
			playerBuilder.SetPBP.subWStatus = ManagedSingleton<StatusHelper>.Instance.GetWeaponStatusX(weaponInfo, 0, bAddStar: false, null, null, tMemberInfo.netSealBattleSettingInfo.PlayerInfo.Exp);
		}
		...
		foreach (NetChipInfo totalChip2 in tMemberInfo.netSealBattleSettingInfo.TotalChipList)
		{
			...
			{
				playerBuilder.SetPBP.tRefPassiveskill.AddPassivesSkill(totalChip2, 4, 1, bCheckUse: true);
			}
			if (totalChip2.ChipID == tMemberInfo.netSealBattleSettingInfo.SubWeaponInfo.Chip)
			{
				playerBuilder.SetPBP.tRefPassiveskill.AddPassivesSkill(totalChip2, 8, 1, bCheckUse: true);
			}
		}
		...
		if (!IsPvp)
		{
			...
			playerBuilder.SetPBP.tPlayerStatus += ManagedSingleton<StatusHelper>.Instance.GetBackupWeaponStatus(bOwner: false, tMemberInfo.netSealBattleSettingInfo.BenchSlotInfoList, tMemberInfo.netSealBattleSettingInfo.BenchWeaponInfoList, tMemberInfo.netSealBattleSettingInfo.WeaponExpertList, tMemberInfo.netSealBattleSettingInfo.WeaponSkillList);
			playerBuilder.SetPBP.tPlayerStatus += ManagedSingleton<StatusHelper>.Instance.GetCardSystemStatus(bOwner: false, tMemberInfo.netSealBattleSettingInfo.CharacterList[0].CharacterID, tMemberInfo.netSealBattleSettingInfo.CharacterList, tMemberInfo.netSealBattleSettingInfo.CardInfoList, tMemberInfo.netSealBattleSettingInfo.CharacterCardSlotInfoList);
		...
		playerBuilder.CreatePlayer(delegate(object[] p_param2)
		{
			...
			if (isLocalPlayer)
			{
				orangeCharacter.gameObject.SetActive(value: true);
			}
			else
			{
	...
	public void RemoveGeneratingPlayer(string sPlayerID)
	{
		...
		if (flag)
		{
			...
			playerByID.PlayerDead(bJustRemove: true);
	...
	public void RebornPlayer(OrangeCharacter oc)
	{
		...
		{
			StageUpdate.SyncStageObj(3, 3, oc.sPlayerID + ",2,0," + origin.x + "," + origin.y + "," + ManagedSingleton<InputStorage>.Instance.GetInputRecordNO(oc.sPlayerID) + ",1", bNoCheckHost: true);
		}
		else
		{
			StageUpdate.SyncStageObj(3, 3, oc.sPlayerID + ",2,0," + origin.x + "," + origin.y + "," + ManagedSingleton<InputStorage>.Instance.GetInputRecordNO(oc.sPlayerID) + ",0", bNoCheckHost: true);
		}
		Singleton<GenericEventManager>.Instance.NotifyEvent(EventManager.ID.STAGE_CONTINUE_PLATER, oc.sPlayerID, p_param2: true, origin.x, origin.y, (bool?)ListPvpIDPosition[index].bLookBack);
	}
				...
				MonoBehaviourSingleton<SignalDispatcher>.Instance.AddHandler(CB.RSBattleLogin, delegate(object res)
				{
					...
					else if (rSBattleLogin.Result == 50052)
					{
						...
						}, delegate
						{
							...
							{
								MonoBehaviourSingleton<UIManager>.Instance.BackToHometop(logoutBattleServer: true, logoutMatchServer: false);
							}
							else
							{
	...
	private void CheckWhenDisconnect()
	{
		...
		{
			Singleton<GenericEventManager>.Instance.NotifyEvent(EventManager.ID.STAGE_UPDATE_HOST, p_param: true);
		}
		else
		{
			Singleton<GenericEventManager>.Instance.NotifyEvent(EventManager.ID.STAGE_UPDATE_HOST, p_param: false);
		}
	}
			...
			MonoBehaviourSingleton<SignalDispatcher>.Instance.AddHandler(CB.RSCreateJoinPVELockedRoom, delegate(object res)
			{
				...
				else if (rSCreateJoinPVELockedRoom.Result != 52000)
				{
					...
					case Code.BATTLE_LOCKEDROOM_CLOSING:
						...
						break;
					default:
						MonoBehaviourSingleton<OrangeGameManager>.Instance.ShowErrorMsg((Code)rSCreateJoinPVELockedRoom.Result, returnTitle: false);
						break;
					}
					BattleServerLogout();
				}
			...
			MonoBehaviourSingleton<SignalDispatcher>.Instance.AddHandler(CB.RSCreateJoinPVPLockedRoom, delegate(object res)
			{
				...
				else if (rSCreateJoinPVPLockedRoom.Result != 52000)
				{
					...
					case Code.BATTLE_LOCKEDROOM_CLOSING:
						...
						break;
					default:
						MonoBehaviourSingleton<OrangeGameManager>.Instance.ShowErrorMsg((Code)rSCreateJoinPVPLockedRoom.Result, returnTitle: false);
						break;
					}
					BattleServerLogout();
					failcb.CheckTargetToInvoke(rSCreateJoinPVPLockedRoom.Result);
	...
	private void AddMemberInfo(ref NTEveryoneJoinedLockedRoom tNTEveryoneJoinedLockedRoom, bool bNeedBorn = false)
	{
		...
		for (int i = 0; i < playeridLength; i++)
		{
			...
			{
				Debug.LogError($"ParserUnsealedBattleSetting Fail! => Length[{playeridLength}] i[{i}] Setting[{tNTEveryoneJoinedLockedRoom.Unsealedbattlesetting(i)}]");
			}
		}
	}
	...
	private void OnNTEveryoneJoinedLockedRoom(object res)
	{
		...
		{
			if (res is NTEveryoneJoinedLockedRoom tNTEveryoneJoinedLockedRoom)
			{
				...
				AddMemberInfo(ref tNTEveryoneJoinedLockedRoom, bNeedBorn: true);
			}
			return;
		}
		if (MonoBehaviourSingleton<OrangeSceneManager>.Instance.NowScene == "StageTest")
		{
			...
			{
				if (!(res is NTEveryoneJoinedLockedRoom tNTEveryoneJoinedLockedRoom2))
				{
					return;
				}
				...
				AddMemberInfo(ref tNTEveryoneJoinedLockedRoom2, bNeedBorn: true);
		...
		if (goStageData.battleType == BattleType.PVP)
		{
			...
			StageUpdate.SetStageName(sTAGE_TABLE.s_STAGE, sTAGE_TABLE.n_DIFFICULTY, bReturnToBootup: false, bGeneratePvePlayer: false, bRegisterPvpPlayer: true);
			int num = 0;
			for (int i = 0; i < ListMemberInfo.Count; i++)
			{
				if (ListMemberInfo[i].PlayerId == MonoBehaviourSingleton<GameServerService>.Instance.PlayerIdentify)
			...
			else if (sTAGE_TABLE.n_MAIN == 90000 && sTAGE_TABLE.n_SUB == 1)
			{
				...
				ManagedSingleton<PlayerNetManager>.Instance.SeasonBattleStartReq(playerBattleSetting, rivalBattleSetting, delegate(SeasonBattleStartRes Res)
				{
					if (Res.Code == 27150)
					{
						MonoBehaviourSingleton<UIManager>.Instance.BackToHometop(logoutBattleServer: true, logoutMatchServer: true, delegate
						{
							MonoBehaviourSingleton<UIManager>.Instance.LoadResourceUI("UI_CommonMsg", delegate(CommonUI ui)
							{
								ui.OpenSE = SystemSE.CRI_SYSTEMSE_SYS_ERROR;
		...
		StageUpdate.SetStageName(sTAGE_TABLE.s_STAGE, sTAGE_TABLE.n_DIFFICULTY, bReturnToBootup: false, bGeneratePvePlayer: true);
		if (bIsReConnect)
		{
			MonoBehaviourSingleton<OrangeSceneManager>.Instance.ChangeScene("StageTest", loadingType);
			return;
		...
		ManagedSingleton<PlayerNetManager>.Instance.StageStartReq(sTAGE_TABLE.n_ID, sTAGE_TABLE.s_STAGE, ManagedSingleton<StageHelper>.Instance.GetStageCrc(sTAGE_TABLE.n_ID), delegate
		{
			MonoBehaviourSingleton<OrangeSceneManager>.Instance.ChangeScene("StageTest", loadingType, null, p_clearSE: false);
		});
	}
	...
	private IEnumerator CheckOCCoroutine(string sPlayerID)
	{
		...
		{
			Singleton<GenericEventManager>.Instance.NotifyEvent(EventManager.ID.PLAYERBUILD_PLAYER_NETCTRLON, sPlayerID, p_param2: true);
	...
	{
		if (res is NTBattleRoomOwnerChange nTBattleRoomOwnerChange)
		{
	...
	private byte[] TransferNTBroadcastBinaryToRoomToByte(NTBroadcastBinaryToRoom ntPro)
	{
		...
		{
			Debug.Log($"Transfer failed. Err=[{ex.Message}].");
	...
	private void PlayerInput(BinaryReader br)
	{
		...
		Singleton<GenericEventManager>.Instance.NotifyEvent(EventManager.ID.PLAYERBUILD_PLAYER_NETCTRLON, text, p_param2: false);
	}
	...
	private string GetSeasonStr(int num)
	{
		return num switch
		{
			1 => MonoBehaviourSingleton<LocalizationManager>.Instance.GetStr("PVP_RANKING_CHARA1"), 
			2 => MonoBehaviourSingleton<LocalizationManager>.Instance.GetStr("PVP_RANKING_CHARA2"), 
			3 => MonoBehaviourSingleton<LocalizationManager>.Instance.GetStr("PVP_RANKING_CHARA3"), 
			_ => string.Empty, 
		};
	}
