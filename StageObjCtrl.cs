...
public class StageObjCtrl : EnemyControllerBase, IManagedUpdateBehavior
{
	...
	private bool bAlwaysStand
	{
		...
		set
		{
			...
			if (_bAlwatsStand)
			{
				...
				_animator.SetBool("bBelow", value: true);
	...
	protected override void Start()
	{
		...
		_animator = GetComponentInChildren<Animator>();
		ModelTransform = OrangeBattleUtility.FindChildRecursive(ref target, "model", equal: true);
		...
		_collideBullet = OrangeBattleUtility.FindChildRecursive(ref target, "ColliderBullet", equal: true).gameObject.AddOrGetComponent<CollideBullet>();
		_collideBullet.isForceSE = (_collideBullet.isBossBullet = true);
		ClawCollideBullet = OrangeBattleUtility.FindChildRecursive(ref target, "ClawCollideBullet", equal: true).gameObject.AddOrGetComponent<CollideBullet>();
		ClawCollideBullet.isForceSE = (ClawCollideBullet.isBossBullet = true);
		Jump_Run_CollideBullet = OrangeBattleUtility.FindChildRecursive(ref target, "ColliderBullet_Jump_Run", equal: true).gameObject.AddOrGetComponent<CollideBullet>();
		Jump_Run_CollideBullet.isForceSE = (Jump_Run_CollideBullet.isBossBullet = true);
		ExplosionRoot = OrangeBattleUtility.FindChildRecursive(ref target, "ExplosionRoot", equal: true);
		if (AILevel == 1)
		{
			...
			LeftAtk = OrangeBattleUtility.FindChildRecursive(ref target, "L_hand_bone4", equal: true);
			RightAtk = OrangeBattleUtility.FindChildRecursive(ref target, "R_hand_bone4", equal: true);
	...
	public void ExploderEffectAndPos()
	{
		...
		if (ExplosionRoot != null)
		{
			...
			{
				StartCoroutine(BossDieFlow(ExplosionRoot, "FX_BOSS_EXPLODE2", showWinPose: false, needStageEnd: false));
			}
			return;
		}
		...
		{
			StartCoroutine(BossDieFlow(targetTF, "FX_BOSS_EXPLODE2", showWinPose: false, needStageEnd: false));
		}
	}
	...
	public void JumpDownAtkOn()
	{
		...
		Jump_Run_CollideBullet.SetBulletAtk(null, selfBuffManager.sBuffStatus, EnemyData);
		Jump_Run_CollideBullet.Active(targetMask);
		Singleton<GenericEventManager>.Instance.NotifyEvent(EventManager.ID.CAMERA_SHAKE, 2f, p_param2: false);
	}
	...
	private void ShootBullet(int id)
	{
		...
		BulletBase bulletBase = BulletBase.TryShotBullet(EnemyWeapons[id].BulletData, ShotPointFs[id - 2], zero.normalized, null, selfBuffManager.sBuffStatus, EnemyData, targetMask, forcePlaySE: true);
		_shootNum++;
