...
internal class OrangeGameManager : MonoBehaviourSingleton<OrangeGameManager>
{
	...
	public string AppVersion
	{
		get
		{
			...
			return $"Ver.{aPP_VERSION} (Source.{ManagedSingleton<ServerConfig>.Instance.PatchVer.ToString().PadRight(3, '0')}{((GetDeviceType() == enums.DeviceType.IOS) ? ApiCommon.ProtocolVersionIOS : ApiCommon.ProtocolVersionAndroid):00}{SocketCommon.ProtocolVersion:00}.{text})";
		}
	}

	public bool IsLvUp { get; set; }

	public bool bLastGamePause => _bLastGamePause;
	...
	public long ServerUnixTimeNowUTC => CapUtility.DateToUnixTime(ServerTimeNowUTC);

	public DateTime ServerTimeNowUTC => DateTime.UtcNow.AddSeconds(serverInfo.ClientDifferTime);

	public long ServerUnixTimeNowLocale => CapUtility.DateToUnixTime(ServerTimeNowLocale);

	public DateTime ServerTimeNowLocale => DateTime.UtcNow.AddHours(serverInfo.TimeZone).AddSeconds(serverInfo.ClientDifferTime);
	...
	private void LateUpdate()
	{
		...
		if (_IsGamePause != _bLastGamePause)
		{
			...
			if (_bLastGamePause)
			{
				Singleton<GenericEventManager>.Instance.NotifyEvent(EventManager.ID.GAME_PAUSE, p_param: true);
				OrangeTimerManager.PauseAll();
			}
			else
			{
				Singleton<GenericEventManager>.Instance.NotifyEvent(EventManager.ID.GAME_PAUSE, p_param: false);
	...
	private void RetrievePlayerInfoCallback(RetrievePlayerInfoRes res)
	{
		...
		{
			ShowErrorMsg((Code)res.Code, returnTitle: false);
		}
	}
	...
	private void CheckLoginRequestComplete()
	{
		...
		if (RequestNow >= RequestMax)
		{
			...
			StartCoroutine(ReDownloadOrangeData(AfterDownloadCallback));
		}
		void AfterDownloadCallback()
		{
			if (GetServerDataCRC() != OrangeDataReader.Instance.GetOrangeDataCRC() || GetServerExDataCRC() != OrangeDataReader.Instance.GetExOrangeDataCRC())
			{
				Debug.LogError($"Planner Client {OrangeDataReader.Instance.GetOrangeDataCRC()} / Server {GetServerDataCRC()}, Operation Client {OrangeDataReader.Instance.GetExOrangeDataCRC()} / Server {GetServerExDataCRC()}!");
			...
			else
			{
				...
				MonoBehaviourSingleton<OrangeCommunityManager>.Instance.OnConnectedEvent += CommunityLoginCallbackEvent;
	...
	public void RetrieveResetTime(Callback p_cb = null)
	{
		...
		ManagedSingleton<PlayerNetManager>.Instance.RetrieveResetTimeReq(delegate(RetrieveResetTimeRes res)
		{
			...
			if (num)
			{
				...
				}, isConfirmParent: true, ignoreTutorial: true);
			}
			else
			{
				p_cb.CheckTargetToInvoke();
	...
	public void SetDesignContentScale()
	{
		if (resolutionInit)
		{
			...
			Debug.Log($"Set Screen Resolution Width:{num} , Height:{num2}");
	...
	{
		CommonUI commonUI = MonoBehaviourSingleton<UIManager>.Instance.LoadResourceUISync<CommonUI>("UI_CommonMsg", isConfirmParent: true, blackBlock: true);
