...
public class CharacterMaterial : MonoBehaviour
{
	...
	public void UpdateProperty(bool setProperty = true)
	{
		if (!isCustomRender)
		{
			...
			{
				mpb.SetFloat(instance.i_Intensity, 0f);
			}
			if (null != SpecColorTex)
			{
				mpb.SetTexture(instance.i_SpecColorTex, SpecColorTex);
			}
			...
			if (Mathf.Abs(DissolveModelHeight) < 2.5f)
			{
				DissolveModelHeight = 2.5f;
			}
	...
	public int Appear(Callback callback = null, float overrideDissolveTime = -1f)
	{
		if (EffectOBJ != null)
		{
			EffectOBJ.gameObject.SetActive(value: true);
		}
		if (isCustomRender)
		{
	...
	public int Disappear(Callback callback = null, float overrideDissolveTime = -1f)
	{
		if (EffectOBJ != null)
		{
			EffectOBJ.gameObject.SetActive(value: false);
		}
		if (isCustomRender)
		{
			callback.CheckTargetToInvoke();
	...
	{
		ClearMaterialTween(callOnComplete: false);
		renderers = null;
	}
