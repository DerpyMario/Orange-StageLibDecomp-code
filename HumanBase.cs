...
public class HumanBase
{
	...
	public enum UniqueBattlePose
	{
		...
		logout,
		win,
		dive_trigger_stand_start,
		dive_trigger_stand_end,
		dive_trigger_jump_start,
		dive_trigger_jump_end,
		dive_trigger_crouch_start,
		dive_trigger_crouch_end,
		logout2
	}
	...
	public static string GetWeaponMotionBundlePath(string animatorType, WeaponType weaponType)
	{
		...
		return $"model/animation/{weaponTypeName}/{animatorType}";
	}
	...
	public static void LoadMotion(StageUpdate.LoadCallBackObj tNewLoad, string animatorType, WeaponType weaponType)
	{
		...
		MonoBehaviourSingleton<AssetsBundleManager>.Instance.LoadAssets(new string[1] { weaponMotionBundlePath }, tNewLoad.LoadCBNoParam, AssetsBundleManager.AssetKeepMode.KEEP_IN_SCENE, p_checkSize: false);
	}
	...
	{
		GetUniqueMotion(out var arg, out var arg2);
	...
	{
		GetUniqueWeaponMotion(out var arg, out var arg2);
		if (arg.Length != 0 && arg[0] == "null")
		{
			return;
		}
