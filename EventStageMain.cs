...
public class EventStageMain : OrangeUIBase
{
	...
	private void AddBossRushEventTab()
	{
		...
		foreach (EVENT_TABLE eventTable in m_bossRushEventTableList)
		{
			...
			newTab.gameObject.SetActive(value: true);
	...
	private void RefreshDifficultyMeter()
	{
		...
		{
			_ = difficultyBarMeter[i];
			if (num3 < m_currentDifficulty)
			{
				m_difficultyBarMeter[num3].gameObject.SetActive(value: true);
			}
			else
			{
				m_difficultyBarMeter[num3].gameObject.SetActive(value: false);
			}
			num3++;
		}
		num3 = 0;
	...
	private void UpdateChallengeCount()
	{
		...
		{
			m_challengeCount.text = $"{availableChallengeCount}/{m_currentStageTable.n_PLAY_COUNT}";
		}
	}
	...
	private void SetChallengeTip(string text)
	{
		...
		{
			m_difficultyDialog.gameObject.SetActive(value: false);
			return;
		}
		m_difficultyDialog.gameObject.SetActive(value: true);
	...
	public void OnClickSweep()
	{
		...
		if (!flag)
		{
			...
			{
				tipUI.Setup(errorMsg, block: true);
			});
			return;
		}
	...
	private bool CheckBeforeStart()
	{
		if (!IsTimedEventAndStartable())
		{
			MonoBehaviourSingleton<UIManager>.Instance.LoadUI("UI_Tip", delegate(TipUI tipUI)
			{
				tipUI.Setup(MonoBehaviourSingleton<LocalizationManager>.Instance.GetStr("NOT_AVAILABLE"), block: true);
			});
			return false;
		}
		...
		if (!ManagedSingleton<StageHelper>.Instance.IsStageConditionOK(m_currentStageTable, ref condition))
		{
			...
			else
			{
				...
				case StageHelper.StageJoinCondition.AP:
				{
					...
					MonoBehaviourSingleton<UIManager>.Instance.LoadUI("UI_Tip", delegate(TipUI tipUI)
					{
						tipUI.Setup(errorMsg2, block: true);
					});
					break;
				}
				}
	...
	private void EventTabHelper(TabType tabType, int tabIndex = 0)
	{
		...
		EnableChallengePanel(bEnable: false);
		...
		case TabType.TIMELIMITED:
		{
			...
			m_energyUseCount.text = $"x{m_currentStageTable.n_AP}";
			EnableBossRushReportPanel(bEnable: false);
			EnableTimedEventReportPanel(bEnable: true);
			...
			m_eventDropPanel.gameObject.SetActive(value: false);
			m_dailyChallengeCountGroup.gameObject.SetActive(value: false);
			m_energyUseGroup.gameObject.SetActive(value: true);
			m_difficultyGroup.gameObject.SetActive(value: true);
		...
		case TabType.BOSSRUSH:
		{
			...
			EnableTimedEventReportPanel(bEnable: false);
			EnableBossRushReportPanel(bEnable: true);
			...
			m_eventDropPanel.gameObject.SetActive(value: false);
			m_dailyChallengeCountGroup.gameObject.SetActive(value: false);
			m_energyUseGroup.gameObject.SetActive(value: false);
			m_difficultyGroup.gameObject.SetActive(value: false);
			m_btnSweep.gameObject.SetActive(value: false);
			bGM_Default = BGM_BossRush;
			break;
		}
		default:
			m_eventTime.text = string.Empty;
			m_rankPanelMini.gameObject.SetActive(value: false);
			EnableTimedEventReportPanel(bEnable: false);
			EnableBossRushReportPanel(bEnable: false);
			m_btnShop.gameObject.SetActive(value: false);
			m_btnTasks.gameObject.SetActive(value: false);
			m_eventDropPanel.gameObject.SetActive(value: true);
			m_dailyChallengeCountGroup.gameObject.SetActive(value: true);
			m_energyUseGroup.gameObject.SetActive(value: false);
			m_difficultyGroup.gameObject.SetActive(value: true);
			m_btnSweep.gameObject.SetActive(value: true);
			bGM_Default = BGM_Default;
			break;
		}
		if (num != 0 && ManagedSingleton<OrangeDataManager>.Instance.EVENT_TABLE_DICT.TryGetValue(num, out var value) && !string.IsNullOrEmpty(value.s_BGM))
		{
	...
	private void UpdateRankingInfoBossRush(EVENT_TABLE eventTable)
	{
		...
		if (ManagedSingleton<PlayerNetManager>.Instance.dicBossRushInfo.ContainsKey(eventTable.n_ID))
		{
			...
			m_bossRushClearTimeMin.text = $"{timeSpan.Minutes:00}";
			m_bossRushClearTimeSec.text = $"{timeSpan.Seconds:00}";
			m_bossRushClearTimeFracSec.text = $"{timeSpan.Milliseconds / 10:00}";
			if (eventTable.n_POINT != 0)
			{
				m_bossRushScore.text = bossRushInfo.netBRInfo.Score.ToString();
				m_bossRushBonus.text = $"{bossRushInfo.netBRInfo.TotalBonus * OrangeConst.BOSSRUSH_BOOST}%";
			}
		}
	...
	private void UpdateRankingInfoTimeLimited(EVENT_TABLE eventTable)
	{
		...
		if (rewardItemID != 0 && ManagedSingleton<ExtendDataHelper>.Instance.ITEM_TABLE_DICT.TryGetValue(rewardItemID, out var value))
		{
			...
			{
				m_limitedEventItems[0].gameObject.SetActive(value: true);
	...
	private int GetEventRewardItemID(EVENT_TABLE eventTable)
	{
		...
		{
			if (ManagedSingleton<OrangeDataManager>.Instance.HOWTOGET_TABLE_DICT.TryGetValue(eventTable.n_SHOP, out var table))
			{
	...
	public void OnClickAddEventEP()
	{
		if (!IsTimedEventAndStartable())
		{
			MonoBehaviourSingleton<UIManager>.Instance.LoadUI("UI_Tip", delegate(TipUI tipUI)
			{
				tipUI.Setup(MonoBehaviourSingleton<LocalizationManager>.Instance.GetStr("CANNOT_BUY_EVENT_STAMINA"), block: true);
			});
			return;
		}
	...
	private void EnableTimedEventReportPanel(bool bEnable, float animTime = 0.2f)
	{
		...
		if (bEnable)
		{
			...
			{
				m_eventReport.gameObject.SetActive(value: true);
		...
		{
			m_eventReport.gameObject.SetActive(value: false);
		})
			.uniqueId;
	}
	...
	private void EnableBossRushReportPanel(bool bEnable, float animTime = 0.2f)
	{
		...
		if (bEnable)
		{
			...
			{
				m_eventReportBossRush.gameObject.SetActive(value: true);
				eventReportCanvasGroup.alpha = alpha;
			}).uniqueId;
			return;
		}
		...
		{
			m_eventReportBossRush.gameObject.SetActive(value: false);
		})
			.uniqueId;
	}
	...
	private void EnableChallengeToggle()
	{
		...
		{
			m_challengeToggleGroup.gameObject.SetActive(value: true);
		}
		else
		{
			m_challengeToggleGroup.gameObject.SetActive(value: false);
		}
	}
	...
	public void OnClickChallengeToggle()
	{
		if (m_currentMode != ModeType.CHALLENGE && m_toggleChallenge.isOn)
		{
			...
			EnableChallengePanel(bEnable: true);
			m_normalModeFX.SetActive(value: false);
			m_challengeModeFX.SetActive(value: true);
			m_btnReward.gameObject.SetActive(IsRewardAvailable());
		}
	}
	...
	public void OnClickNormalToggle()
	{
		if (m_currentMode != 0 && m_toggleNormal.isOn)
		{
			...
			EnableChallengePanel(bEnable: false);
			m_normalModeFX.SetActive(value: true);
			m_challengeModeFX.SetActive(value: false);
			m_btnReward.gameObject.SetActive(IsRewardAvailable());
		}
	}
