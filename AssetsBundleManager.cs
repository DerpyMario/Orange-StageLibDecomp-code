...
public class AssetsBundleManager : MonoBehaviourSingleton<AssetsBundleManager>
{
	...
	public bool IsLoadManifest => isLoadManifest;
	...
	private IEnumerator OnStartLoadManifest()
	{
		...
		if (manifest == null)
		{
			Debug.LogError($"Can't get manifest...PATH : [{manifestPath}]");
		}
		else
		{
	...
	{
		MonoBehaviourSingleton<UIManager>.Instance.UpdateLoadingBlock(p_active: true);
	...
	private IEnumerator OnStartLoadAssets(string[] abName, Callback p_cb, AssetKeepMode p_keepMode = AssetKeepMode.KEEP_IN_SCENE, bool p_checkSize = true)
	{
		...
		p_cb?.Invoke();
		MonoBehaviourSingleton<UIManager>.Instance.UpdateLoadingBlock(p_active: false);
		Singleton<GenericEventManager>.Instance.NotifyEvent(EventManager.ID.UPDATE_DOWNLOAD_BAR, p_param: false);
	}
	...
	private IEnumerator OnStartLoadSingleAsset(string bundleName, AssetKeepMode keepMode = AssetKeepMode.KEEP_IN_SCENE)
	{
		...
		if (!isLoaded)
		{
			...
			if (!dictBundleInfo.ContainsKey(bundleName))
			{
				...
				{
					lastSaveCount = -1;
					Singleton<GenericEventManager>.Instance.NotifyEvent(EventManager.ID.UPDATE_DOWNLOAD_BAR, p_param: true);
	...
	public void LoadAllAssetBundle(Callback p_cb)
	{
		...
		if (totalFileSize == 0L)
		{
			...
			{
				tipUI.Setup(MonoBehaviourSingleton<LocalizationManager>.Instance.GetStr("TIP_ALL_DOWNLOADED_IS_NEW"), block: true);
	...
	private IEnumerator OnLoadAllAssetBundle(Callback p_cb)
	{
		...
		}, AssetKeepMode.KEEP_NO, p_checkSize: false);
		yield return CoroutineDefine._waitForEndOfFrame;
	}
	...
	private IEnumerator OnStartUnloadBundleCache(Callback p_cb, bool forceClear)
	{
		...
		foreach (string key in list)
		{
			...
			{
				assetbundleInfo.Bundle.Unload(unloadAllLoadedObjects: false);
	...
	{
		if (dictBundleInfo.TryGetValue(p_bundleName, out var value))
		{
